...
class UserController {
    ...
    async register(req, res) {
        try {
            ...
            res.status(response.status).location(`/users/${response.data.insertedId}`).json({ ok: true, message: 'success (user registered in the database)', insertedDocument: response.data, redirect: "/index.html" });
        } catch (err) {
